...
* Download the csv of the dataset from [the course website](https://math-141.github.io/labs/lab_09.html).

Then within the RStudio Server project:

* In the lower-right window, click on the "Files" tab.
* Navigate to the location in your home directory where you want to store the dataset.  (I recommend storing it in the same place as your lab.)
* Then click "Upload".
...
* From the console, grab the line of code that includes `read_csv()` and insert it below.
    + Try rendering the document.  If it won't render because it can't find the csv, make you the path you have entered in the **relative path** in relation to the Quarto document you are working in. 
